...
if grep -q "isMinifyEnabled = false" "$BUILD_GRADLE_FILE"; then
  ...
  echo "✓ 已启用 R8 优化: isMinifyEnabled = true"
...
# 确保启用资源压缩
if grep -q "isShrinkResources = false" "$BUILD_GRADLE_FILE"; then
  sed -i 's/isShrinkResources = false/isShrinkResources = true/g' "$BUILD_GRADLE_FILE"
  echo "✓ 已启用资源压缩: isShrinkResources = true"
elif grep -q "isShrinkResources = true" "$BUILD_GRADLE_FILE"; then
  echo "✓ 资源压缩已启用"
else
  echo "注意: 建议在 build.gradle.kts 中添加 isShrinkResources = true"
fi

# 检查是否使用了优化版本的 proguard 配置
if grep -q "proguard-android-optimize.txt" "$BUILD_GRADLE_FILE"; then
  echo "✓ 已使用优化版 ProGuard 配置"
else
  echo "建议: 使用 'proguard-android-optimize.txt' 而非 'proguard-android.txt'"
  echo "  在 build.gradle.kts 中修改为: getDefaultProguardFile('proguard-android-optimize.txt')"
fi
...
# 创建优化的 gradle.properties 配置建议
GRADLE_PROPERTIES_FILE="V2rayNG/gradle.properties"

echo "创建性能优化的 gradle.properties 配置建议..."

# 定义性能优化的 gradle 配置
PERFORMANCE_GRADLE_CONFIG="# Performance Optimized Gradle Configuration
# Generated by refactor_package.sh for V2rayNG
# 
# 使用方法：
# 1. 备份现有的 gradle.properties
# 2. 将以下配置合并到 V2rayNG/gradle.properties 中
# 3. 或者直接使用此文件替换（注意保留项目特定配置）
...
# org.gradle.buildcache.remote.enabled=false
...
# 将配置写入建议文件
echo "$PERFORMANCE_GRADLE_CONFIG" > "$GRADLE_PROPERTIES_FILE"
echo "✓ 性能优化配置已创建: $GRADLE_PROPERTIES_FILE"

# 验证最终配置
echo "验证 R8 性能优化配置..."
if [ -f "$PROGUARD_FILE" ] && grep -q "isMinifyEnabled = true" "$BUILD_GRADLE_FILE"; then
  echo "✓ R8 性能优化配置完成并已启用"
  
  # 检查关键配置
  if grep -q "dontobfuscate" "$PROGUARD_FILE"; then
    echo "✓ 混淆已禁用（-dontobfuscate）"
  fi
  
  if grep -q "optimizationpasses" "$PROGUARD_FILE"; then
    echo "✓ 多轮优化已启用"
  fi
  
  if grep -q "assumenosideeffects.*Log" "$PROGUARD_FILE"; then
    echo "✓ 调试日志移除已配置"
  fi
else
  echo "⚠ R8 优化配置可能不完整，请检查"
fi
